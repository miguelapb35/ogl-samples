...
int drawArrays(int argc, char* argv[], csv & CSV)
{
	...
	{
		testDrawArrays Test(argc, argv, framework::CORE, TEST_FRAME_PER_RUN,
...
int drawElements(int argc, char* argv[], csv & CSV)
{
	...
	for(std::size_t TestIndex(0); TestIndex < TEST_DUPLICATE_COUNT; ++TestIndex)
	{
		testDrawElements Test(argc, argv, framework::CORE, TEST_FRAME_PER_RUN,
...
int drawArraysVAOs(int argc, char* argv[], csv & CSV)
{
	...
	for(std::size_t TestIndex(0); TestIndex < TEST_DUPLICATE_COUNT; ++TestIndex)
	{
		testDrawArraysVAO Test(argc, argv, framework::CORE, TEST_FRAME_PER_RUN,
...
*/
	...
	for(std::size_t TestIndex(0); TestIndex < TEST_DUPLICATE_COUNT; ++TestIndex)
	{
		testScreenspaceCoherence Test(
			argc, argv, framework::CORE, TEST_FRAME_PER_RUN,
...
int compiler(int argc, char* argv[], csv & CSV)
{
	...
	{
		testCompiler Test(argc, argv, framework::CORE, TEST_FRAME_PER_RUN, testCompiler::MULTITHREADED);
		Error += Test();
		Test.log(CSV, "Multithreaded GLSL compiler");
	}

	{
		testCompiler Test(argc, argv, framework::CORE, TEST_FRAME_PER_RUN, testCompiler::DUALTHREADED);
		Error += Test();
		Test.log(CSV, "Dualthreaded GLSL compiler");
	}

	{
		testCompiler Test(argc, argv, framework::CORE, TEST_FRAME_PER_RUN, testCompiler::SINGLETHREADED);
		Error += Test();
