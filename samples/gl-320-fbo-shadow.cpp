...
namespace
{
	...
	std::string const FRAG_SHADER_SOURCE_DEPTH(glf::DATA_DIRECTORY + "gl-320/fbo-shadow-depth.frag");
	...
	std::string const VERT_SHADER_SOURCE_SPLASH(glf::DATA_DIRECTORY + "gl-320/fbo-shadow-splash.vert");
	std::string const FRAG_SHADER_SOURCE_SPLASH(glf::DATA_DIRECTORY + "gl-320/fbo-shadow-splash.frag");
...
bool initProgram()
{
	...
	if(Validated)
	{
		...
		GLuint VertShaderName = Compiler.create(GL_VERTEX_SHADER, VERT_SHADER_SOURCE_DEPTH, 
			"--version 150 --profile core");
		GLuint FragShaderName = Compiler.create(GL_FRAGMENT_SHADER, FRAG_SHADER_SOURCE_DEPTH,
			"--version 150 --profile core");
		Validated = Validated && Compiler.check();
